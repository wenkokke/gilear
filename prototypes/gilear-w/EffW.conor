...
data (&>) (s :: Time)(t :: Time) :: Type where
  Now  :: t &> t -- t hasn't become better
  -- why is it `Step s t` and not `s &> t` ?
  -- do we want/need the evidence of stepping to be linked to the actual language?
  (:<) :: r &> s -> Step s t -> r &> t
...
-- homomorphism between steps and composition of indexed types
...
-- exists u such that if t &> u then v u ?
-- if timed type v can be timed with t, then there must exist
-- a time u where v can be timed with.
-- did u happened before t ?
type Kripke v t = forall u. LE t u => v u

-- if v is Timed, v is kripked for all times
kripke :: Timed v => All (v -:> Kripke v)
kripke v = v &> lesson

-- if v is Timed, then I can get an time-indexed f by kripkefying v
kripkefy :: Timed v => All (v -:> (Kripke v -:> f) -:> f)
-- why is it called k ?
kripkefy v k = k (kripke v)

-- Time Monad: arrow monad from timed-types to timed-types
...
-- Inx n, where n is the number of variables in scope
...
-- n keeps track how many variables depend on it
data Ty (n :: N)
  = E ExVar    -- existential type variables
  | U (Inx n)  -- all variables that dependend on U
  | Ty n :-> Ty n
  deriving (Show, Eq)
...
-- Tyme is closed at all times
newtype Tyme (t :: Time) = Ty (Ty Z) deriving (Show, Eq)

-- if I have a constant existential, I can embed it in Tyme
tiE :: All (K ExVar -:> Tyme)
tiE (K e) = Ty (E e)

-- breaks out of Kripke, as Tyme is constant
...
unArrEh (Ty (s :-> t)) =
  kripkefy (Ty s) $ \ s -> kripkefy (Ty t) $ \ t -> UnArrAye s t
unArrEh _ = UnArrNaw
...
-- Type Scheme: mono or poly
data Sch (n :: N)
  = T (Ty n)
  | P (Sch (S n))
  deriving Show

-- Scheme that is constant for all times
newtype Schime (t :: Time) = Sch (Sch Z) deriving Show

-- embedding Ty into Sch
monotype :: All (Tyme -:> Schime)
monotype (Ty t) = Sch (T t)

-- is Ty n a dependency?
dep :: ExVar -> Ty n -> Bool
dep x (E y) = x == y
...
gen (K e) (Sch s) =
  case go Zi s of
    -- b flags whether generalisation has happened
    (s', b) -> if b then Sch (P s') else Sch s
 where
...
data FTiMo
  ...
  (s :: Time)
  :: Type where
  RetNow :: v s -> FTiMo c v s
  Call :: forall r
        . c r s
       -> All ((&>) s -:> r -:> FTiMo c v)
       -> FTiMo c v s

instance (forall r. Timed (c r), Timed v) => Timed (FTiMo c v) where
...
instance MoTime (FTiMo c) where
  retNow = RetNow
  (>>>=) :: forall c f g s. (Timed f, Timed g)
         => FTiMo c f s
         -> (forall t. (SegTo t, SegFrom t ~ s) =>
             Kripke f t -> FTiMo c g t)
         -> FTiMo c g s
  RetNow v >>>= k = leap (now v) (k (kripke v))
  Call c j >>>= k = Call c $ \ u r -> j u r >>>= jump u seg
    where
      jump :: forall t t'. s &> t -> t &> t'
           -> Kripke f t' -> FTiMo c g t'
      jump u w f = leap (u &> w) (k f)

op :: Timed r => c r s -> FTiMo c r s
...
-- handle Next...
nonce :: Timed v => Int -> All (FTiMo W v -:> FTiMo W v)
...
fresh :: String -> FTiMo W (K ExVar) i
fresh x =
  op Next >>>= \ i ->
  retNow (kmap (x,) i)
...
type ProgVar = String
-- handle VSch...
decl :: Timed v => ProgVar -> All (Schime -:> FTiMo W v -:> FTiMo W v)
...
bloc :: All (FTiMo W Tyme -:> FTiMo W Schime)
...
guessing :: All (K ExVar -:> FTiMo W Schime -:> FTiMo W Schime)
...
funTy :: All (Tyme -:> FTiMo W (Tyme :* Tyme))
...
-- guess a type
guess :: All (FTiMo W Tyme)
guess = op (Inst (Sch (P (T (U Zi)))))

-- make types the same!
unify :: All (Tyme -:> Tyme -:> FTiMo W (K ()))
